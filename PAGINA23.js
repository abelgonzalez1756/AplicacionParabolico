...
var colorBackground = "#ffff00";                           // Hintergrundfarbe   - color de fondo
var colorGround = "#ffc040";                               // Farbe für Untergrund -  color para la superficie
var colorPosition = "#ff0000";                             // Farbe für Position   - colr para posicionar
var colorVelocity = "#ff00ff";                             // Farbe für Geschwindigkeit - color para la velocidad
var colorAcceleration = "#0000ff";                         // Farbe für Beschleunigung - color para la aceleracion

var colorAngle = "#00ffff";                                // Farbe für Winkelmarkierung  - color para marcar  angulo
colorClock1 = "#808080";                                   // Farbe für Gehäuse der Digitaluhr - color para la carcasa del relog digital
colorClock2 = "#000000";                                   // Farbe für Anzeige der Digitaluhr - color para la visiualizacion del reloj digital
colorClock3 = "#ff0000";                                   // Farbe für Ziffern der Digitaluhr -  color para digitos del reloj digital

// Sonstige Konstanten:

var PI2 = 2*Math.PI;                                       // Abkürzung für 2 pi - abreviatura de 2pi
var DEG = Math.PI/180;                                     // 1 Grad (Bogenmaß)  -  1grado  ( Radianes)
var FONT1 = "normal normal bold 12px sans-serif";          // Normaler Zeichensatz  -  conjunto de  caracteres normales
var FONT2 = "normal normal bold 16px monospace";           // Zeichensatz für Digitaluhr - juego de caracteres para el reloj digital
var xU = 50, yU = 270;                                     // Position des Ursprungs (Pixel)- posicion de origen (pixel)

// Attribute:

var canvas, ctx;                                           // Zeichenfläche, Grafikkontext  - mesa de trabajo contexto grafico
var width, height;                                         // Abmessungen der Zeichenfläche (Pixel) - dimensiones del area de dibujo
var bu1, bu2;                                              // Schaltknöpfe (Zurück, Start)
var cbSlow;                                                // Optionsfeld (Zeitlupe)  Botón de radio (cámara lenta)
var ipH, ipV, ipW, ipG;                                    // Eingabefelder   Campos de entrada    
var rbY, rbV, rbA ;                               // Radiobuttons   Botones de radio
var on;                                                    // Flag für Bewegung    bandera para movimiento
var slow;                                                  // Flag für Zeitlupe     marcar  para camara lenta
var t0;                                                    // Anfangszeitpunkt       hora de inicio
var t;                                                     // Aktuelle Zeit (s)     hora actual 
var timer;                                                 // Timer für Animation    temporizador de animacion

var g;                                                     // Fallbeschleunigung (m/s²)   aceleracion gravedad (m/s²)
var h0;                                                    // Ausgangshöhe (m)   altura inicial 
var v0, v0x, v0y;                                          // Anfangsgeschwindigkeit (Betrag und Komponenten, m/s) - componentes velocidades
var alpha0;                                                // Wurfwinkel (Bogenmaß)    angulo de lanzamiento

var x, y;                                                  // Koordinaten (m)   Cordenadas (metros)
var x0, y0;                                                // Koordinaten (Pixel)     coordeenadas (pixele)
var pix;                                                   // Umrechnungsfaktor (Pixel pro Meter) factor de conversion
var tW;                                                    // Flugdauer (s)  duracion de vuelo
var w;                                                     // Wurfweite (m)  rango de tiro
var hMax;                                                  // Maximale Höhe (m)     altura maxima
var vyMax;                                                 // Maximum von vy (Betrag, m/s)   velocidad maxima
                                                   
var nrSize;                                                // Dargestellte physikalische Größe cantidad fisica representada
var pos1, pos2;                               // Textpositionen (je nach Sprache) posicion de texto segun idioma   extensiones (dependiendo del idioma) posicion de texto segun idioma
...
function getElement (id, text) {
  var e = document.getElementById(id);                     // Element        elemento
  if (text) e.innerHTML = text;                            // Text festlegen, falls definiert   Establecer texto si está definido  
  return e;                                                // Rückgabewert    Valor de retorno
  } 
  
// Start:

function start () {
  canvas = getElement("cv");                               // Zeichenfläche    mesa de trabajo
  width = canvas.width; height = canvas.height;            // Abmessungen (Pixel)     dimensiones
  ctx = canvas.getContext("2d");                           // Grafikkontext   contexto grafico
  bu1 = getElement("bu1",text01);                          // Resetknopf    boton reinicio
  bu2 = getElement("bu2",text02[0]);                       // Startknopf    boton de inicio
  bu2.state = 0;                                           // Zunächst Zustand vor dem Start  primero declarar antes del comienzo
  cbSlow = getElement("cbSlow");                           // Optionsfeld Zeitlupe      boton de radio de camar alenta
  cbSlow.checked = false;                                  // Zeitlupe zunächst abgeschaltet   La cámara lenta se apagó primero
 
  getElement("lbSlow",text03);                             // Erklärender Text (Zeitlupe)   Texto explicativo (cámara lenta)
  getElement("ipHa",text04);                               // Erklärender Text (Ausgangshöhe)  Texto explicativo (altura inicial)
  ipH = getElement("ipHb");                                // Eingabefeld (Ausgangshöhe)    Campo de entrada (altura inicial)
  getElement("ipHc",meter);                                // Einheit (Ausgangshöhe)       Unidad (altura inicial)     
  getElement("ipVa",text05);                               // Erkärender Text (Anfangsgeschwindigkeit)   Texto explicativo (velocidad inicial)
 
  ipV = getElement("ipVb");                                // Eingabefeld (Anfangsgeschwindigkeit)  Campo de entrada (velocidad inicial)
  
  getElement("ipVc",metrosporsegundo);                       // Einheit (Anfangsgeschwindigkeit)    Unidad (velocidad inicial)

  getElement("ipWa",text06);                               // Erklärender Text (Winkel)  Texto explicativo angulo
  ipW = getElement("ipWb");                                // Eingabefeld (Winkel)  campo de entrada angulo
  getElement("ipWc",degree);                               // Einheit (Winkel)     unidad angulo
  getElement("ipMa",text07);                               // Erklärender Text (Masse)    tExto expliactivo masivo      
  ipM = getElement("ipMb");                                // Eingabefeld (Masse)    campo de entrada masa
  getElement("ipMc",kilogram);                             // Einheit (Masse)      unidad ( masa)
  getElement("ipGa",text08);                               // Erklärender Text (Fallbeschleunigung)  texto explicativo aceleracion de la graveadad
  ipG = getElement("ipGb");                                // Eingabefeld (Fallbeschleunigung)     Campo de entrada (aceleración gravitacional) 
  getElement("ipGc",meterPerSecond2);                      // Einheit (Fallbeschleunigung) uindad aceleracion de la gravedad
  rbY = getElement("rbY");                                 // Radiobutton (Position)     boton de radio (posicion) 
  getElement("lbY",text09);                                // Erklärender Text (Position)      texto explicativo posicion
  rbV = getElement("rbV");                                 // Radiobutton (Geschwindigkeit)    boton de radio velocidad
  getElement("lbV",text10);                                // Erklärender Text (Geschwindigkeit)   texto explicativo velocidad
  rbA = getElement("rbA");                                 // Radiobutton (Beschleunigung)      boton radio aceleracion  
  getElement("lbA",text11);                                // Erklärender Text (Beschleunigung) texto exoplicativo aceleracion
  
 
  rbY.checked = true; nrSize = 1;                          // Zunächst Position ausgewählt    primera posicion seleccionadad
  getElement("author",author);                             // Autor (eventuell Übersetzer) auitor posiblememnte traductor
    
  on = slow = false;                                       // Animation und Zeitlupe zunächst abgeschaltet  animacion y la camra lenta se apaga primero
  h0 = 5; v0 = 5; alpha0 = 45*DEG; m = 1; g = 9.81;        // Anfangswerte der Eingabefelder     valores iniciales de los campos de entrada
  updateInput();                                           // Eingabefelder aktualisieren  a  ctualizar campo de entrada
  enableInput(true);                                       // Eingabe zunächst möglich    entrada posible primero
  t = 0;                                                   // Zeitvariable (s)   variable de tiempo (s)
  calculation();                                           // Berechnungen (Seiteneffekt!)     calculos (efectos secundarios)
  calcPos12();                                             // Berechnung von pos1 und pos2    calculos de pos1 y pos2
  paint();                                                 // Zeichnen    dibujo
  bu1.onclick = reactionReset;                             // Reaktion auf Schaltknopf (Zurück)   respuesta al boton atras
  bu2.onclick = reactionStart;                             // Reaktion auf Schaltknopf (Start/Pause/Weiter) espuest al boton (inicio/pauasa/continuar)
  cbSlow.onclick = reactionSlow;                           // Reaktion auf Optionsfeld (Zeitlupe)     respuesta al boton camara lenta
  ipH.onkeydown = reactionEnter;                           // Reaktion auf Enter-Taste (Eingabefeld Ausgangshöhe)    Reacción a la tecla Intro (altura de salida del campo de entrada) 
...
function setButton2State (st) {
  bu2.state = st;                                          // Zustand speichern       guardar  estado
  bu2.innerHTML = text02[st];                              // Text aktualisieren      actualizar texto
  }
// Umschalten des Schaltknopfs Start/Pause/Weiter:
  
function switchButton2 () {
  var st = bu2.state;                                      // Momentaner Zustand     estado actual
...
function reactionStart () {
  switchButton2();                                         // Zustand des Schaltknopfs ändern  cambiar estado del boton
...
function startAnimation () {
  on = true;                                               // Animation angeschaltet   animacion activada
...
function stopAnimation () {
  on = false;                                              // Animation abgeschaltet  animacion apagada
...
function calcPos12 () {
  ctx.font = FONT1;                                        // Normaler Zeichensatz    Conjunto de caracteres normal
...
// Schrittweite für beschriftete Markierungen (in m):     Incremento para marcas etiquetadas (en m)
...
function calculation () {
  v0x = v0*Math.cos(alpha0);                               // Waagrechte Geschwindigkeitskomponente am Anfang (m/s)  Componente de velocidad horizontal al principio (m / s)
...
function newPath () {
  ctx.beginPath();                                         // Neuer Grafikpfad        Nueva ruta de gráficos                   
  ctx.strokeStyle = "#000000";                             // Linienfarbe schwarz     Color de linea negro
...
function line (x1, y1, x2, y2, c, w) {
  newPath();                                               // Neuer Grafikpfad (Standardwerte)   Nueva ruta de gráficos (valores predeterminados)
  if (c) ctx.strokeStyle = c;                              // Linienfarbe festlegen     Establecer color de línea
  if (w) ctx.lineWidth = w;                                // Liniendicke festlegen     Establecer grosor de línea
  ctx.moveTo(x1,y1); ctx.lineTo(x2,y2);                    // Linie vorbereiten         Preparar la línea
  ctx.stroke();                                            // Linie zeichnen              Dibujar una linea
  }
  
// Rechteck mit schwarzem Rand:                                  Rectángulo con un borde negro:
// (x,y) ... Koordinaten der Ecke links oben (Pixel)             Coordenadas de la esquina superior izquierda (píxeles)
// w ....... Breite (Pixel)                                      w ....... ancho (píxeles)
// h ....... Höhe (Pixel)                                        h ....... altura (píxeles)
// c ....... Füllfarbe (optional)                                c ....... color de relleno (opcional)
...
// Kreisscheibe mit schwarzem Rand:                      Disco circular con borde negro:
// (x,y) ... Mittelpunktskoordinaten (Pixel)             (x, y) ... coordenadas del centro (píxeles)
// r ....... Radius (Pixel)                              r ....... radio (píxel)
// c ....... Füllfarbe (optional)                        c ....... color de relleno (opcional)

function circle (x, y, r, c) {
  if (c) ctx.fillStyle = c;                                // Füllfarbe        Color de relleno
  newPath();                                               // Neuer Pfad       nuevo camino
...
function arrow (x1, y1, x2, y2, w) {
  if (!w) w = 1;                                           // Falls Liniendicke nicht definiert, Defaultwert   Si el grosor de la línea no está definido, valor predeterminado                     
  var dx = x2-x1, dy = y2-y1;                              // Vektorkoordinaten            coordenadas vectoriales
  var length = Math.sqrt(dx*dx+dy*dy);                     // Länge         Longitud
  if (length == 0) return;                                 // Abbruch, falls Länge 0         Terminación si longitud 0
  dx /= length; dy /= length;                              // Einheitsvektor                  Vector de unidad
  var s = 2.5*w+7.5;                                       // Länge der Pfeilspitze           Longitud de la punta de flecha
  var xSp = x2-s*dx, ySp = y2-s*dy;                        // Hilfspunkt für Pfeilspitze      Punto de ayuda para punta de flecha      
  var h = 0.5*w+3.5;                                       // Halbe Breite der Pfeilspitze     La mitad del ancho de la punta de flecha
  var xSp1 = xSp-h*dy, ySp1 = ySp+h*dx;                    // Ecke der Pfeilspitze             Esquina de punta de flecha
...
function orbit () {
  newPath();                                             //  Nueva ruta de gráficos (valores predeterminados) Neuer Grafikpfad (Standardwerte)   
  ctx.strokeStyle = colorPosition;                      // Farbe für Position     Color para posicionar
  ...
  ctx.moveTo(xx,yy);                                       // Anfangspunkt des Polygonzugs         Punto de partida de la polilínea
  var t = 0;                                               // Zeitvariable (s)                     Variable de tiempo (s)    
  while (t < tW) {                                         // Solange Wurf nicht zu Ende ...       Mientras la basura no termine ...
...
function velocity () {
  var vy = v0y-g*t;                                        // Senkrechte Geschwindigkeitskomponente (m/s)     Componente de velocidad vertical (m / s)
  var alpha = Math.atan(vy/v0x);                           // Neigungswinkel (Bogenmaß)                  Ángulo de inclinación (radianes)  
  angle(x0,y0,alpha);                                      // Winkelmarkierung   marca de angulo
  orbit();                                                 // Wurfbahn
  var l = vyMax*10;                                        // Maximale Pfeillänge, falls 10 Pixel für 1 m/s
  var f = (l>120 ? l/120 : 1);                             // Faktor (mindestens gleich 1)
  var dxPix = v0x*10/f;                                    // Waagrechte Komponente (Pixel)    componete horizontal (pixel)
  var dyPix = vy*10/f;                                     // Senkrechte Komponente (Pixel) 
  ctx.strokeStyle = colorVelocity;                         // Farbe für Geschwindigkeit
  arrow(x0,y0,x0+dxPix,y0-dyPix,3);                        // Dicker Pfeil für Geschwindigkeit       gran flecha para la velocidad
  arrow(x0,y0,x0+dxPix,y0);                                // Dünner Pfeil für waagrechte Komponente   flecha delgada para componete horizontal
...
function paint () {
  ctx.fillStyle = colorBackground;                         // Hintergrundfarbe      color de fondo
  ctx.fillRect(0,0,width,height);                          // Hintergrund ausfüllen     completa el fondo
  if (on) {                                                // Falls Animation angeschaltet ...     si la animacion esta activada
    var t1 = new Date();                                   // ... Aktuelle Zeit
    var dt = (t1-t0)/1000;                                 // ... Länge des Zeitintervalls (s)  duracion del intervalo de tiempo
